...
local function freestyle(bar)
	...
	local bg = bar:Get("bigwigs:Qulightui:bg")
	if bg then
		bg:ClearAllPoints()
	...
	local ibg = bar:Get("bigwigs:Qulightui:ibg")
	if ibg then
		ibg:ClearAllPoints()
...
local applystyle = function(bar)
	...
	bg:Show()
	bar:Set("bigwigs:Qulightui:bg", bg)
	...
	if bar.candyBarIconFrame:GetTexture() then
		...
		ibg:Show()
		bar:Set("bigwigs:Qulightui:ibg", ibg)
	end
...
local function registerStyle(myProfile)
	...
	bars:RegisterBarStyle("QulightUI", {
		apiVersion = 1,
		version = 1,
		GetSpacing = function() return T.Scale(13) end,
		ApplyStyle = applystyle,
		BarStopped = freestyle,
		GetStyleName = function() return "QulightUI" end,
	})
...
function T.UploadBW()
	...
	local bars = BigWigs:GetPlugin("Bars", true)
	if bars then
		bars.db.profile.barStyle = "QulightUI"
