...
local function LoadSkin()
	...
	InfoFrame.ArtBackdrop:SetFrameLevel(InfoFrame:GetFrameLevel())
	...
	hooksecurefunc(ChromieTimeExpansionButtonMixin, "SetupButton", function(self)
		if not self.ArtBackdrop then
			...
			self.ArtBackdrop:SetFrameLevel(self:GetFrameLevel())
			...
			if self.SetHighlightTexture then
				...
				highligh:SetPoint("TOPLEFT", self.Background, 0, 0)
				highligh:SetPoint("BOTTOMRIGHT", self.Background, 0, 0)
