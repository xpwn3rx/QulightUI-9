...
local MainBars = function()
	if C.actionbar.split_bars ~= true then
		...
		elseif QulightUISettingsPerChar.BottomBars == 2 then
			...
			Bar2Holder:Show()
			Bar5Holder:Hide()
		elseif QulightUISettingsPerChar.BottomBars == 3 then
...
local RightBars = function()
	if C.actionbar.rightbars > 2 then
		if QulightUISettingsPerChar.RightBars == 1 then
			...
			Bar3Holder:Hide()
			Bar4Holder:Hide()
		elseif QulightUISettingsPerChar.RightBars == 2 then
			...
			Bar3Holder:Hide()
			Bar4Holder:Show()
		elseif QulightUISettingsPerChar.RightBars == 3 then
			...
			Bar4Holder:Show()
			Bar6Holder:Show()
		elseif QulightUISettingsPerChar.RightBars == 0 then
			if not C.actionbar.petbar_horizontal == true then
				PetActionBarAnchor:ClearAllPoints()
...
local SplitBars = function()
	if C.actionbar.split_bars == true then
		if QulightUISettingsPerChar.SplitBars == true then
			...
			SplitBarLeft:Show()
			SplitBarRight:Show()
		elseif QulightUISettingsPerChar.SplitBars == false then
			ToggleBar[3]:ClearAllPoints()
...
local LockCheck = function(i)
	if QulightUISettingsPerChar.BarsLocked == true then
		ToggleBar[i].Text:SetText("U")
		ToggleBar[i].Text:SetTextColor(0.3, 0.3, 0.9)
	elseif QulightUISettingsPerChar.BarsLocked == false then
...
for i = 1, 5 do
	...
	elseif i == 2 then
		...
		ToggleBar[i]:SetScript("OnMouseDown", function()
			if InCombatLockdown() then print("|cffffff00"..ERR_NOT_IN_COMBAT.."|r") return end
			QulightUISettingsPerChar.RightBars = QulightUISettingsPerChar.RightBars - 1

			if C.actionbar.rightbars > 2 then
				if QulightUISettingsPerChar.RightBars > 3 then
					QulightUISettingsPerChar.RightBars = 2
				elseif QulightUISettingsPerChar.RightBars > 2 then
					QulightUISettingsPerChar.RightBars = 1
				elseif QulightUISettingsPerChar.RightBars < 0 then
					QulightUISettingsPerChar.RightBars = 3
				end
			elseif C.actionbar.rightbars < 3 then
				if QulightUISettingsPerChar.RightBars > 2 then
					QulightUISettingsPerChar.RightBars = 1
				elseif QulightUISettingsPerChar.RightBars < 0 then
					QulightUISettingsPerChar.RightBars = 2
				end
	...
	elseif i == 5 then
		...
		ToggleBar[i]:SetScript("OnMouseDown", function()
			...
			elseif QulightUISettingsPerChar.BarsLocked == false then
				QulightUISettingsPerChar.BarsLocked = true
			end
	...
	if i == 3 or i == 4 then
		ToggleBar[i]:SetScript("OnMouseDown", function()
			...
			if QulightUISettingsPerChar.SplitBars == false then
				QulightUISettingsPerChar.SplitBars = true
			elseif QulightUISettingsPerChar.SplitBars == true then
				QulightUISettingsPerChar.SplitBars = false
			end
			SplitBars()
		end)
	...
	ToggleBar[i]:SetScript("OnEnter", function()
		...
		if i == 5 then
			...
			GameTooltip:AddDoubleLine(" ", TALENT_TREE_LOCKED..": "..(QulightUISettingsPerChar.BarsLocked and "|cff55ff55"..L_STATS_ON or "|cffff5555"..strupper(OFF)), 1, 1, 1, 0.75, 0.90, 1)
			GameTooltip:Show()
		end
	end)
