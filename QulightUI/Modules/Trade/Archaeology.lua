...
function stArch:OnLoad(self)
	...
	if QulightUISettingsPerChar.Archaeology == false then self:Hide() end
	-- Title Bar
	...
	stArch["close"]:SetScript("OnMouseUp", function() self:Hide() QulightUISettingsPerChar.Archaeology = false end)
...
SlashCmdList.STARCHAEOLOGIST = function()
	...
	if stArchFrame:IsShown() then QulightUISettingsPerChar.Archaeology = true else QulightUISettingsPerChar.Archaeology = false end
end
...
stArchFrame:HookScript("OnEvent", function(self)
	...
	if not select(3, GetProfessions()) then stArchFrame:Hide() QulightUISettingsPerChar.Archaeology = false end
	if QulightUISettingsPerChar.Archaeology == true and not self:IsShown() then self:Show() end
end)
...
b:SetScript("OnClick", function()
	if _G["stArchaeologyFrame"]:IsShown() then
		...
		QulightUISettingsPerChar.Archaeology = true
	end
