...
local restoreDefaultPosition = function(style, identifier, obj)
	...
	if QulightUIPositions.Default[target:GetName()] then
		target:ClearAllPoints()
		target:SetPoint(unpack(QulightUIPositions.Default[target:GetName()]))
		local backdrop = backdropPool[target]
		if backdrop then
			backdrop:ClearAllPoints()
			backdrop:SetAllPoints(target)
		end
		QulightUIPositions.Default[target:GetName()] = nil
		QulightUIPositions.UnitFrame[target:GetName()] = nil
	else
...
local function restorePosition(obj)
	...
	local style, identifier, isHeader = getObjectInformation(obj)
	local target = isHeader or obj
	if QulightUIPositions.UnitFrame[target:GetName()] then
		local SetPoint = getmetatable(target).__index.SetPoint
...
local saveDefaultPosition = function(obj)
	...
	if not QulightUIPositions.Default[target:GetName()] then
		if not p then
			p = UIParent
		end
		QulightUIPositions.Default[target:GetName()] = {ap, p:GetName(), rp, x, y}
	end
end
...
do
	...
	function frame:VARIABLES_LOADED()
		...
		_DB = QulightUIPositions.UFPos or {}
		QulightUIPositions.UFPos = _DB

		QulightUIPositions.UnitFrame = QulightUIPositions.UnitFrame or {}
...
StaticPopupDialogs.RESET_UF = {
	...
	button1 = ACCEPT,
	button2 = CANCEL,
	OnAccept = function() if InCombatLockdown() then print("|cffffff00"..ERR_NOT_IN_COMBAT.."|r") else QulightUIPositions.UFPos = {} QulightUIPositions.UnitFrame = {} ReloadUI() end end,
	timeout = 0,
	whileDead = 1,
	hideOnEscape = true,
	preferredIndex = 5,
