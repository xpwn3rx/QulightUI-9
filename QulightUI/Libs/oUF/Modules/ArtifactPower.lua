...
local function OnMouseUp(element, btn)
	...
	if btn == "MiddleButton" then
		if element.offAlpha == 0 then
			element.offAlpha = 1
			QulightUISettings.ArtifactPower = true
		else
			element.offAlpha = 0
			QulightUISettings.ArtifactPower = false
		end
	else
...
local function CheckAlpha(element)
	if QulightUISettings and QulightUISettings.ArtifactPower == true then
		element.offAlpha = 1
...
local function UpdateColor(element, isUsable)
	...
	element.bg:SetVertexColor(color[1], color[2], color[3], 0.2)	-- QulightUI
end
