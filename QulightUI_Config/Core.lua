...
local baseName = "QulightUIOptionsPanel"

-- [[ Functions ]]

-- when an option needs a reload
local function setReloadNeeded(isNeeded)
	QulightUIOptionsPanel.reloadText:SetShown(isNeeded)
	ns.needReload = isNeeded -- for the popup when clicking okay
	if isNeeded then
		QulightUIOptionsPanelOkayButton:Enable()
	else
		QulightUIOptionsPanelOkayButton:Disable()
	end
end
...
local function CreateOptionPanel(name, text, subText)
	local panel = CreateFrame("Frame", name, QulightUIOptionsPanel)
...
init:SetScript("OnEvent", function()
	...
	local QulightUIOptionsPanel = QulightUIOptionsPanel

	StaticPopupDialogs.PERCHAR = {
		text = L_GUI_PER_CHAR,
		OnAccept = function()
			if QulightUIOptionsPanel.ProfileBox:GetChecked() then
				QulightUIOptionsGlobal[realm][name] = true
			else
		...
		OnCancel = function()
			if QulightUIOptionsPanel.ProfileBox:GetChecked() then
				QulightUIOptionsPanel.ProfileBox:SetChecked(false)
			else
				QulightUIOptionsPanel.ProfileBox:SetChecked(true)
			end
		end,
		button1 = ACCEPT,
		button2 = CANCEL,
	...
	QulightUIOptionsPanel.ProfileBox:SetChecked(QulightUIOptionsGlobal[realm][name])
	QulightUIOptionsPanel.ProfileBox:SetScript("OnClick", function()
		StaticPopup_Show("PERCHAR")
	end)

	QulightUIOptionsPanel:SetTemplate("Transparent")

	local sunFrame = CreateFrame("Frame", nil, QulightUIOptionsPanel)
	...
	T.SkinCheckBox(QulightUIOptionsPanel.ProfileBox)
